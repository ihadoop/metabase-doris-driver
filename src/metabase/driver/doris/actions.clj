(ns metabase.driver.doris.actions
  "Method impls for [[metabase.driver.sql-jdbc.actions]] for `:doris."
  (:require
...
(defmethod sql-jdbc.actions/maybe-parse-sql-error [:doris actions.error/violate-not-null-constraint]
...
(defmethod sql-jdbc.actions/maybe-parse-sql-error [:doris actions.error/violate-unique-constraint]
...
(defmethod sql-jdbc.actions/maybe-parse-sql-error [:doris actions.error/violate-foreign-key-constraint]
...
(defmethod sql-jdbc.actions/maybe-parse-sql-error [:doris actions.error/incorrect-value-type]
  [_driver error-type _database _action-type error-message]
...
;;; cf https://dev.doris.com/doc/refman/8.0/en/data-types.html
;;; et https://dev.doris.com/doc/refman/5.7/en/data-types.html
;;; vs https://dev.doris.com/doc/refman/5.7/en/cast-functions.html#function_cast
;;; et https://dev.doris.com/doc/refman/8.0/en/cast-functions.html#function_cast
(defmethod sql-jdbc.actions/base-type->sql-type-map :doris
  [_driver]
  {:type/Date           "DATE"
...
;;; doris doesn't need to do anything special with nested transactions; the
;;; original transaction can proceed even if some specific statement errored.
(defmethod sql-jdbc.actions/do-nested-transaction :doris
  [_driver _conn thunk]
  (thunk))
...
;;; doris returns the generated ID (of which cannot be more than one)
...
(defmethod sql-jdbc.actions/select-created-row :doris
