...
(defn- kill! [conn pid]
  (let [results (sql.ddl/jdbc-query conn ["show processlist"])
    ...
    (when result?
      ...
      (throw (Exception. (trs "Killed doris process id {0} due to timeout." pid))))))
...
(defmethod ddl.i/unpersist! :doris
...
(defmethod ddl.i/check-can-persist :doris
                ...
                (fn create-kv-table [conn]
                  (sql.ddl/execute! conn (sql/format
                                          (ddl.i/populate-kv-table-honey-sql-form
                                           schema-name)
                                          {:dialect :doris})))]]]
    ;; Unlike postgres, doris ddl clauses will not rollback in a transaction.
